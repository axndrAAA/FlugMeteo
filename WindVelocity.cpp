...
float WindVelocity::readVabs()
{
   return speed.readLinearV(TACHYMETER_RADIUS);
}
...
WindVelocity::Wind WindVelocity::readVvector()
{
...
void WindVelocity::init(uint8_t tach_pin)
{
    init_tachymeter(tach_pin);
...
void WindVelocity::init_tachymeter(uint8_t tach_pin)
{
    speed = Tachymeter(tach_pin);
    speed.init();
}